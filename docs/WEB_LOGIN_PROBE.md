...
更新：2026-09-06。此工程是授权路径验证工具，不是已经可用的抽卡记录客户端。
...
2026-09-06，用户报告通过 GitHub Pages 验证入口打开官方用户中心后，可以登录并看到已绑定的 B服终末地角色。该结果确认当前账号的“官网可登录、B服终末地绑定可见”前提；未收集设备/浏览器版本，开发侧未访问登录态、Cookie、Token、UID 或网络响应，因此不升级 grant → binding → U8 或寻访接口状态。下一实验仅确认官方页面是否存在公开、用户主动触发的授权或记录入口；不得要求用户提交开发者工具中的秘密值。

## U-003 官网入口观察

2026-09-06，用户在可见的绑定页面中未发现“寻访记录”“抽卡记录”或类似入口。该结果关闭“直接从官网点击公开寻访入口”的当前账号验证分支，但不证明官方内部接口不存在。纯 GitHub Pages 站点不能借此取得其他域的授权数据。下一技术验证转为：由 GitHub Actions 构建不含秘密的 Android 登录壳 PoC，手机只下载安装产物；在确认 Android WebView 安全边界前，不实现 Token 捕获或真实取数。
