...
更新：2026-09-06。本文管理产品定义；具体技术主题由下方文档负责，避免复制多份相互漂移的规范。
...
按 D-015，当前优先验证 Android 官方网页登录与 B服绑定授权，URL 入口保留。2026-09-06 用户 U-002 已在手机浏览器登录官网并看到绑定的 B服终末地角色；U-003 随后确认可见页面没有寻访/抽卡记录入口。这些是页面可见性的用户实测，不代表网页能够返回授权凭据或拉取寻访记录，也未开发 APK。现有共享核心与本地存储目标保持适用。
...
原型实现与最新环境证据见 [WEB_LOGIN_PROBE](WEB_LOGIN_PROBE.md)。

## 当前仓库事实

- 初次检查为空目录，已初始化独立 Git 仓库 main；不是 bhaoo 的 fork。已选择性移植其 Nuxt 配置，尚未导入第三方游戏业务实现。
- 2026-09-04 新增 Nuxt/Tauri 最小网页登录观察原型及构建脚本，配置衍生来源已登记。没有用户数据库或真实凭据；尚未接入授权返回和记录同步。
...
- 已有用户在手机浏览器登录官网并看到绑定 B服终末地角色的观察证据；开发侧未接触其会话。完整授权返回、取数、落库、URL 取得方式、Token 生命周期和唯一性规则仍未 VERIFIED。
