...
状态：源码线索 PARTIAL，Android 完整链 UNKNOWN；已有手机浏览器页面与绑定可见性的用户实测节点，尚无授权返回或取数 VERIFIED 节点。状态解释以 [API_RESEARCH](API_RESEARCH.md) 为准。

## 当前优先验证路线

用户 U-001 报告断网后仅“网络异常”、无完整地址，渠道和版本待确认。2026-09-06，用户 U-002 报告可在手机浏览器登录官方用户中心，并看到已绑定的 B服终末地角色；随后 U-003 报告页面没有“寻访记录”“抽卡记录”或类似入口。未提供秘密数据，开发侧未读取会话。当前优先调查下方官方网页登录与绑定流程能否在 Android 完成，先验证登录页面、绑定前提和安全授权返回，再接共享同步核心。不能假设网页登录已经支持 B服或已有可安装 APK。
...
| 官方网页登录与绑定可见性 | USER-OBSERVED | U-002 已看到 B服终末地角色；设备/版本及开发侧独立复现待补 |
| 官网用户可见的寻访入口 | USER-OBSERVED-ABSENT | U-003 未发现寻访/抽卡记录入口；不能泛化为官方没有任何内部接口 |
